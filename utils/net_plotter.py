...
def setup_direction(args, dir_file, net):
    ...
    if exists(dir_file):
        ...
        if 'ydirection' in f.keys() and 'xdirection' in f.keys():
            f.close()
...
def setup_surface(args, surf_file, dir_file):
    print('-------------------------------------------------------------------')
    print('setup_surface')
    print('-------------------------------------------------------------------')
    # skip if the direction file already exists
    if os.path.exists(surf_file):
        ...
        if 'ycoordinates' in f.keys() and 'xcoordinates' in f.keys():
            f.close()
            print ("%s is already set up" % surf_file)
            return

    f = h5py.File(surf_file, 'a')
    print("Setting up the plotting surface...")
    f['dir_file'] = dir_file
